...
// --- Custom Base64 implementation for BCRYPT ---
const BCRYPT_ALPHABET: &[u8] = b"./ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789";
...
pub fn base64_encode(src: &[u8]) -> String {
    ...
    while i < src.len() {
        ...
        if i + 2 <= src.len() {
            out.push(BCRYPT_ALPHABET[((chunk >> 6) & 0x3F) as usize] as char);
        }
        if i + 3 <= src.len() {
            out.push(BCRYPT_ALPHABET[(chunk & 0x3F) as usize] as char);
        }
        
        i += 3;
    }
    // Truncate/pad to expected bcrypt lengths (salt is 22 chars for 16 bytes, hash is 31 chars for 23 bytes)
    if src.len() == 16 {
        out.truncate(22);
    } else if src.len() == 23 {
        out.truncate(31);
...
pub fn base64_decode(src: &str, dest: &mut [u8]) -> Result<(), &'static str> {
    let bytes = src.as_bytes();
    let mut dest_idx = 0;
    
    // For Bcrypt, we decode exactly the 6-bit chunks
    let mut buffer = 0u32;
    let mut bits = 0;
    
    for &b in bytes {
        let idx = char_to_index(b).ok_or("Invalid character in base64")? as u32;
        buffer = (buffer << 6) | idx;
        bits += 6;
        if bits >= 8 {
            bits -= 8;
            if dest_idx < dest.len() {
                dest[dest_idx] = ((buffer >> bits) & 0xFF) as u8;
                dest_idx += 1;
            }
        }
    }
    ...
    if dest_idx < dest.len() {
        return Err("Insufficient base64 data");
    }
...
impl fmt::Display for BcryptError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match *self {
            BcryptError::CostNotAllowed(c) => write!(f, "Cost needs to be between 4 and 31, got {}", c),
            BcryptError::InvalidHash(r) => write!(f, "Invalid hash: {}", r),
            BcryptError::Truncation(l) => write!(f, "Expected 72 bytes or fewer; found {} bytes", l),
        }
    }
}
...
fn hash_password_internal(
    ...
    let output = bcrypt(cost, salt, truncated);
    
    let salt_b64 = base64_encode(&salt);
    let hash_b64 = base64_encode(&output[..23]);
    
    Ok(format!("$2b${:02}${}{}", cost, salt_b64, hash_b64))
}

pub fn hash<P: AsRef<[u8]>>(password: P, cost: u32) -> BcryptResult<String> {
    let mut salt = [0u8; 16];
    crate::rand::fill_bytes(&mut salt);
    hash_password_internal(password.as_ref(), cost, salt, false)
}

pub fn verify<P: AsRef<[u8]>>(password: P, hash_str: &str) -> BcryptResult<bool> {
    if hash_str.len() != 60 {
        return Err(BcryptError::InvalidHash("expected 60 bytes"));
    }
    let bytes = hash_str.as_bytes();
    if bytes[0] != b'$' || bytes[3] != b'$' || bytes[6] != b'$' {
        return Err(BcryptError::InvalidHash("malformed hash"));
    }
    let version = &hash_str[1..3];
    if version != "2y" && version != "2b" && version != "2a" && version != "2x" {
        return Err(BcryptError::InvalidHash("invalid bcrypt prefix"));
    }
    let cost = hash_str[4..6]
        .parse::<u32>()
        .map_err(|_| BcryptError::InvalidHash("invalid cost"))?;
        
    let salt_b64 = &hash_str[7..29];
    let hash_b64 = &hash_str[29..60];
    
    let mut salt = [0u8; 16];
    base64_decode(salt_b64, &mut salt).map_err(|_| BcryptError::InvalidHash("invalid salt base64"))?;
    
    let mut expected_hash = [0u8; 23];
    base64_decode(hash_b64, &mut expected_hash).map_err(|_| BcryptError::InvalidHash("invalid hash base64"))?;
    
    // Hash password using extracted salt
    let password_len = password.as_ref().len();
    let copy_len = password_len.min(72);
    let mut pass = [0u8; 72];
    pass[..copy_len].copy_from_slice(&password.as_ref()[..copy_len]);
    let used = (copy_len + 1).min(72);
    let truncated = &pass[..used];
    
    let generated = bcrypt(cost, salt, truncated);
    
    Ok(constant_time_eq(&generated[..23], &expected_hash))
}
