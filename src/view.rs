...
// 1. Load Static Assets from Internal Resources (Embedded in Library)
...
pub static JINJA: LazyLock<Environment<'static>> = LazyLock::new(|| {
    ...
    // Default Loader: Hanya mencari di disk (untuk development)
    // Untuk production, app harus menambahkan template secara manual ke JINJA
    env.set_loader(|name| {
...
// 3. Fungsi Helper untuk Render HTML
...
fn render_internal(template: &str, context: minijinja::Value) -> Response {
    ...
    match JINJA.get_template(template) {
        ...
        Err(err) => {
            ...
            (StatusCode::NOT_FOUND, format!("Template Not Found: {}", err)).into_response()
        }
    }
}
